...
```bash
# Run this below to curl the SMD dataset into your build repo.
BASE=https://raw.githubusercontent.com/NetManAIOps/OmniAnomaly/master/ServerMachineDataset
for s in train test test_label; do
  curl -sSL --create-dirs -o ../data/SMD/$s/machine-1-1.txt $BASE/$s/machine-1-1.txt

cd code #navigate to the code directory

pip install -r requirements.txt
# If you see PyTorch / NumPy compatibility issues, install NumPy 1.x:
# pip install 'numpy<2'
...
### LSTM VAE (500 epochs, test-stride 5) with small input projection layer and and optional drop out layer

| model          | F1     | precision | recall | F1 (PA) | ROC-AUC | PR-AUC | params | train_s | infer_s |
| -------------- | ------ | --------- | ------ | ------- | ------- | ------ | ------ | ------- | ------- |
| LSTM-VAE - SMD | 0.6152 | 0.5907    | 0.6418 | 0.9983  | 0.9014  | 0.5779 | 74.7k  | 975.2   | 3.60    |
...
1. **Can a diffusion model trained on normal data find anomalies?**
   Yes ROC-AUC is around 0.96–0.97 on SMD from training on the normal split only.
2. **How does the denoising strategy shape the normal-vs-anomaly gap?**
   It does matter as vanilla leads raw F1 on SMD but, DDPM-selective gives the best
   recall at the lowest inference cost with a strong masking but has computationally expensive.
   On smooth synthetic data the ordering flips to selective > masking > vanilla.
3. **Does the gain justify the cost?**
   The cost columns quantify it: 4× training and up to ~260× inference overhead versus the VAE is worth it on SMD but not on the
   easy synthetic set.
